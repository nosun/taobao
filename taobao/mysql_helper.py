...
class MysqlHelper(object):
    ...
    def get_tmall_shops(self, shop_id=None):
        c = self.db.cursor(pymysql.cursors.DictCursor)
        if shop_id:
            sql = "select * from shops where task_url like '%tmall%' and id = %s" % shop_id
        else:
            sql = "select * from shops where task_url like '%tmall%'"
        c.execute(sql)
        return c.fetchall()
    ...
    def format_data(self, item):
        ...
        item['sizes'] = pymysql.escape_string(repr(item['sizes']))
        item['colors'] = pymysql.escape_string(repr(item['colors']))
        item['prices'] = pymysql.escape_string(repr(item['prices']))
...
if __name__ == "__main__":
    ...
    sql = """insert into products(sn, title, url, thumb, price, sid, created_at, updated_at)
    values (%s, %s, %s, %s, %s, %s, %s, %s)"""
    params = ('5387819359971', 'c',
              'b',
              'a',
              '28.00', '5', '2017-10-31 16:35:45', '2017-10-31 16:35:45')
    c = db.db.cursor()
